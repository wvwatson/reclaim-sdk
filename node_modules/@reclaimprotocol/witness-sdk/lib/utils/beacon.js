...
exports.makeBeaconCacheable = exports.assertValidClaimOwner = exports.makeOwnerProof = exports.getWitnessIdFromPrivateKey = exports.fetchWitnessListForClaim = void 0;
...
async function makeOwnerProof(request, privateKey) {
    const serialised = api_1.InitialiseSessionRequest_BeaconBasedProviderClaimRequest
        .encode({ ...request, ownerProof: undefined })
        .finish();
    const signature = await signatures_1.SelectedServiceSignature.sign(serialised, privateKey);
    const address = await signatures_1.SelectedServiceSignature.getAddress(await signatures_1.SelectedServiceSignature
        .getPublicKey(privateKey));
    return { signature, address };
}
exports.makeOwnerProof = makeOwnerProof;
/**
 * Verify that the person who is wanting to claim
 * actually is making the claim
 */
async function assertValidClaimOwner(request) {
    const { ownerProof } = request;
    const serialised = api_1.InitialiseSessionRequest_BeaconBasedProviderClaimRequest
        .encode({ ...request, ownerProof: undefined })
        .finish();
    const verified = await signatures_1.SelectedServiceSignature.verify(serialised, ownerProof.signature, ownerProof.address);
    if (!verified) {
        throw new Error('Invalid claim owner signature');
    }
}
exports.assertValidClaimOwner = assertValidClaimOwner;
